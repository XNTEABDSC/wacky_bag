...
use crate::structures::{
    ...
    n_dim_array::{
        ...
        n_dim_indexer_u::NDimIndexerU,
        t_n_dim_array::{
            TNDimArray, TNDimArrayForEach, TNDimArrayForEachEdge, TNDimArrayForEachEdgeParallel, TNDimArrayForEachParallel, TNDimArrayIterPair, TNDimArrayIterPairParallel
        },
    },
};
...
fn merge_index<const DIM: usize>(
    chunks_idx: &NDimIndex<DIM>,
    elem_idx: &NDimIndex<DIM>,
    dim_elem_count: usize,
) -> NDimIndex<DIM> {
    array::from_fn(|dim| chunks_idx[dim] * (dim_elem_count as isize) + elem_idx[dim])
}
fn split_index<const DIM: usize>(
    indexes: &NDimIndex<DIM>,
    dim_elem_count: usize,
) -> (NDimIndex<DIM>, NDimIndex<DIM>) {
    let split_data = indexes.map(|a| {
        let a = a as usize;
        (a / dim_elem_count, a % dim_elem_count)
    });
    let (chunks_idx, inner_idx) = (
        split_data.map(|(a, _)| a as isize),
        split_data.map(|(_, b)| b as isize),
    );
    return (chunks_idx, inner_idx);
}

impl<const DIM: usize, T> NDimChunkArray<DIM, T> {
    /// create [NDimChunkArray] by function
    pub fn from_fn(
        lens: [usize; DIM],
        dim_elem_count: usize,
    ...
    /// when iterating inner chunk, whether parallel will be used.
    /// true if there are less than 8 chunk
    pub fn parallel_iterating_chunk(&self) -> bool {
        self.values.values().len() < 8
    }
}
...
impl<const DIM: usize, T> TNDimArray<DIM, T> for NDimChunkArray<DIM, T> {
    ...
    fn get_mut(&mut self, indexes: &super::n_dim_index::NDimIndex<DIM>) -> Option<&mut T> {
        ...
        self.values
            .get_mut(&chunks_idx)
            .and_then(|a| a.get_mut(&inner_idx))
    }
}

impl<const DIM: usize, T> TNDimArrayForEach<DIM, T> for NDimChunkArray<DIM, T> {
    fn for_each<'env, Func>(&'env self, func: &mut Func)
    where
        Func: FnMut(&'env T, NDimIndex<DIM>),
        T: 'env,
    {
        self.values.for_each(&mut |a, chunks_idx| {
            a.for_each(&mut |b, elem_idx| {
    ...
    where
        ...
        T: 'env,
    {
        self.values.for_each_mut(&mut |chunk, chunks_idx| {
            chunk.for_each_mut(&mut |item, elem_idx| {
                let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
                func(item, idx);
            });
        });
...
impl<const DIM: usize, T> TNDimArrayForEachParallel<DIM, T> for NDimChunkArray<DIM, T> {
    fn for_each_parallel<'ext_env, Func, TScopeCreator>(
        ...
        T: Send + Sync,
    {
        let par_inner = self.parallel_iterating_chunk();
        self.values.for_each_parallel(
            &|a, chunks_idx| {
                if par_inner {
                    a.for_each_parallel(
                        &|b, elem_idx| {
    ...
    {
        let par_inner = self.parallel_iterating_chunk();
        self.values.for_each_mut_parallel(
            &|chunk, chunks_idx| {
                if par_inner {
                    chunk.for_each_mut_parallel(
                        &|item, elem_idx| {
                            let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
                            func(item, idx);
                        },
...
impl<const DIM: usize, T> TNDimArrayForEachEdge<DIM, T> for NDimChunkArray<DIM, T> {
	fn for_each_edge<'ext_env, Func>(&'ext_env self, dim_dir: super::dim_dir::DimDir, func: &mut Func)
	where
		Func: FnMut(&'ext_env T, NDimIndex<DIM>),
		T: Send + Sync + 'ext_env {
		self.values.for_each_edge(dim_dir, &mut |c,chunks_idx|{
			c.for_each_edge(dim_dir, &mut |v,elem_idx|{
                let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
                func(v, idx);
			});
		});
	...
	fn for_each_edge_mut<'ext_env, Func>(&'ext_env mut self, dim_dir: super::dim_dir::DimDir, func: &mut Func)
	where
		Func: FnMut(&'ext_env mut T, NDimIndex<DIM>),
		T: Send + Sync + 'ext_env {
		self.values.for_each_edge_mut(dim_dir, &mut |c,chunks_idx|{
			c.for_each_edge_mut(dim_dir, &mut |v,elem_idx|{
                let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
                func(v, idx);
			});
		});
...
impl<const DIM: usize, T> TNDimArrayForEachEdgeParallel<DIM, T> for NDimChunkArray<DIM, T> {
	fn for_each_edge_parallel<'ext_env, Func, TScopeCreator>(
		...
		TScopeCreator: crate::traits::scope_no_ret::ThreadScopeCreator + Sync,
		T: Send + Sync {
		let par_iter=self.parallel_iterating_chunk();
		self.values.for_each_edge_parallel(dim_dir, &|c,chunks_idx|{
			if par_iter {
				c.for_each_edge_parallel(dim_dir, &|v,elem_idx|{
					let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
					func(v, idx);
				},scope_creator);
			}else {
	...
	fn for_each_edge_mut_parallel<'ext_env, Func, TScopeCreator>(
		...
		T: Send + Sync {
		let par_iter=self.parallel_iterating_chunk();
		self.values.for_each_edge_mut_parallel(dim_dir, &|c,chunks_idx|{
			if par_iter {
				c.for_each_edge_mut_parallel(dim_dir, &|v,elem_idx|{
					let idx = merge_index(&chunks_idx, &elem_idx, self.dim_elem_count);
					func(v, idx);
				},scope_creator);
			}else {
...
impl<const DIM: usize, T> TNDimArrayIterPair<DIM, T> for NDimChunkArray<DIM, T> {
    ...
    where
        ...
        T: 'ext_env,
    {
        self.values.for_each(&mut |chunk, _| {
            chunk.iter_pair(&mut |a, b, dim| {
                func(a, b, dim);
            });
        });
        self.values.iter_pair(&mut |chunka, chunkb, dim| {
            ...
            loop {
                let a_item = unsafe { transmute(&a_uw.1[a_idx_op.get_compressed()]) };
                let b_item = unsafe { transmute(&b_uw.1[b_idx_op.get_compressed()]) };
                ...
                if b_idx_op.get()[dim] != 0 {
                    ...
                    if 1 <= dim {
                        ...
                        if c != 0 {
                            ...
                            debug_assert_eq!(
                                dim_carry, true,
                                "not synchronous carry of 2 index iterating"
                            );
                            break;
                        }
                    } else {
    ...
    where
        ...
        T: 'ext_env,
    {
        self.values.for_each_mut(&mut |chunk, _| {
            chunk.iter_pair_mut(&mut |a, b, dim| {
                func(a, b, dim);
            });
        });
        ...
        >,
            ...
            .unwrap();
            let mut b_idx_op =
                NDimIndexOperator::from_index(b_uw.0.deref().deref(), [0; DIM]).unwrap();

            loop {
                let a_item = unsafe { transmute(&mut a_uw.1[a_idx_op.get_compressed()]) };
                let b_item = unsafe { transmute(&mut b_uw.1[b_idx_op.get_compressed()]) };
                func(a_item, b_item, dim);
                // func(&mut a_uw.1[a_idx_op.get_compressed()],&mut b_uw.1[b_idx_op.get_compressed()],dim);
                let c = a_idx_op.move_n_carry(1);
                if c != 0 {
...
impl<const DIM: usize, T> TNDimArrayIterPairParallel<DIM, T> for NDimChunkArray<DIM, T> {
        ...
        T: Send + Sync,
    {
        ...
        self.values.iter_pair_parallel(
            &|chunka: &NDimArray<Owned<NDimIndexerU<DIM>>, DIM, T, Vec<T>>, chunkb, dim| {
                ...
                loop {
                    let a_item = &a_uw.1[a_idx_op.get_compressed()];
                    let b_item = &b_uw.1[b_idx_op.get_compressed()];
                    func(a_item, b_item, dim);
                    // func(&mut a_uw.1[a_idx_op.get_compressed()],&mut b_uw.1[b_idx_op.get_compressed()],dim);
                    let c = a_idx_op.move_n_carry(1);
                    if c != 0 {
...
mod test {
    ...
    fn test_chunks_lens() {
        let chunk_array =
            NDimChunkArray::<2, i32>::from_fn([10, 15], 4, |idx| (idx[0] * 100 + idx[1]) as i32);
    ...
    fn test() {
        ...
        for i in 0..10 {
            for j in 0..15 {
                assert_eq!(
                    *chunk_array.get(&[i as isize, j as isize]).unwrap(),
                    (i * 100 + j) as i32
                );
            }
        }
