...
{
    fn use_scope(&mut self, scope:&'scope TScope)->();
}
...
impl ThreadScopeCreator for ThreadScopeCreatorStd {
    ...
    fn scope<'env,Func>(f:&mut Func ) -> ()
        ...
        where Func: for<'scope> ThreadScopeUser<'scope,thread::Scope<'scope,'env>>,
            ...
            //'env:'scope
    {
        thread::scope(move |s|f.use_scope(s));
    }
...
mod test{
    ...
    fn test(){
        ...
        {
            let mut dwa=AScopeUser{a:&a,x:&mut x};
		    let _spam=ThreadScopeCreatorStd::scope(&mut dwa);
        }
